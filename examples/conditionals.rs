...
fn main() {
    ...
    let var = if 1 < rand_num && rand_num < 10 {
        ...
        2000 // placing a ";" at the end of a returned value WITHOUT the "return" keyword will return the unit value "()" instead
    } else {
        0
    };
    println!("var is: {var}");

    let x = match rand_num { // "match" must be *exhaustive*
        1..=30 => 3,
        31..=61 => 6,
