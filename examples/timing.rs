...
fn loop1(size: u64) -> u64 {
    ...
    let mut sum = 0;
    for v in vec {
        // ownership of the elements in "vec" is moved into the loop variable "v"
...
fn loop3(size: u64) -> u64 {
    ...
    let mut sum = 0;
    for &v in &vec {
        // dereferences the referenced elements from the vector "vec", essentially creating a copy of each element
        sum += v // adds values directly
    }
    sum
}
