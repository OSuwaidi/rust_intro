...
fn main() {
    ...
    for e in &mut vec2 { // here "e" is a mutable reference (of type &mut i32) to each element in the vector "vec2"
        // must assign "e" a *reference* to "vec2", otherwise, "vec2"'s ownership will be moved to the loop
        *e *= 2; // the "*" in the beginning of the variable allows you to manipulate the reference's ("vec2") values inplace
    }
    for &e in &vec2 { // here "e" is a copy (of type i32) of each element in the vector "vec2"
        println!("{e}");
    }
